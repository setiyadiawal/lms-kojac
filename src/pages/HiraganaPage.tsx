...
type Filter = 'all' | HiraganaVariant;
...
const variantLabels: Record<HiraganaVariant, string> = {
  basic: 'Dasar',
  dakuten: 'Dakuten',
...
function itemSetKey(items: HiraganaWithProgress[]) {
  return items.map((item) => item.id).join('|');
}

export function HiraganaPage() {
  const { items, stats, loading, error, recordReview } = useHiragana();
  ...
  return <div className="page kana-page">
    <div className="kana-breadcrumb"><Link to="/belajar"><ArrowLeft size={16}/> Belajar</Link><span>/</span><strong>Hiragana</strong></div>
    <div className="page-header kana-header">
      <div><p className="eyebrow">MILESTONE 2A · HURUF JEPANG</p><h1>ひらがな <span>Hiragana</span></h1><p>Pelajari bentuk, bunyi, lalu kuatkan ingatan dengan flashcard dan quiz.</p></div>
    ...
    {error && <div className="notice">Gagal memuat Hiragana: {error}. Pastikan migration <strong>002_hiragana_learning.sql</strong> sudah dijalankan.</div>}
    ...
    {loading ? <KanaLoading /> : <>
      {tab === 'study' && <StudyView items={filtered} />}
...
function StudyView({ items }: { items: HiraganaWithProgress[] }) {
  ...
  return <div className="study-layout">
    <aside className="kana-detail-card">
      ...
      <HiraganaStrokeOrder text={selected.prompt} />
...
type ChartRow = {
  key: string;
  label: string;
  columns: Array<HiraganaWithProgress | null>;
};
...
const basicFiveColumnRows: Array<{ key: string; label: string; groups: Array<string | null> }> = [
  ...
  { key: 'w', label: 'W / ん', groups: ['w', 'n-special'] },
];

function bySortOrder(a: HiraganaWithProgress, b: HiraganaWithProgress) {
  return (a.extra?.sort_order ?? 9999) - (b.extra?.sort_order ?? 9999);
}

function basicRows(items: HiraganaWithProgress[]): ChartRow[] {
...
function groupedRows(items: HiraganaWithProgress[], variant: HiraganaVariant): ChartRow[] {
...
function KanaStudyChart({ items, selectedId, onSelect }: StudyChartProps) {
  const variants = new Set(items.map((item) => (item.extra?.variant ?? 'basic') as HiraganaVariant));
  const sections: Array<{ variant: HiraganaVariant; title: string; columns: 3 | 5; rows: ChartRow[] }> = [];

  if (variants.has('basic')) sections.push({ variant: 'basic', title: 'Hiragana Dasar', columns: 5, rows: basicRows(items) });
  ...
  return <div className="kana-study-chart">
    {sections.map((section) => <div className={`kana-chart-section chart-${section.variant}`} key={section.variant}>
      {sections.length > 1 && <div className="kana-chart-title"><strong>{section.title}</strong><span>{section.variant === 'yoon' ? 'ゃ · ゅ · ょ' : 'a · i · u · e · o'}</span></div>}
...
function KanaCell({ item, selected, onSelect }: { item: HiraganaWithProgress; selected: boolean; onSelect: (id: string) => void }) {
...
function FlashcardView({ items, onRate }: { items: HiraganaWithProgress[]; onRate: (id: string, rating: 0 | 1 | 2 | 3) => Promise<unknown> }) {
  ...
  const variant = variantLabels[(card.extra?.variant ?? 'basic') as HiraganaVariant];
...
function buildFlashcardDeck(items: HiraganaWithProgress[]) {
  const now = Date.now();
  return [...items]
    .sort((a, b) => {
...
const quizModes: Array<{ mode: QuizMode; title: string; description: string }> = [
  { mode: 'kana-romaji', title: 'Kana → Romaji', description: 'Pilih romaji yang sesuai dengan Hiragana.' },
  { mode: 'romaji-kana', title: 'Romaji → Kana', description: 'Pilih Hiragana berdasarkan romaji.' },
  { mode: 'typing', title: 'Ketik Jawaban', description: 'Ketik romaji dari Hiragana yang tampil.' },
  { mode: 'audio', title: 'Dengarkan & Pilih', description: 'Dengarkan bunyi lalu pilih Hiragana yang benar.' },
  { mode: 'matching', title: 'Matching', description: 'Cocokkan Kana dan romaji dalam satu ronde.' },
  { mode: 'mixed', title: 'Quiz Campuran', description: 'Jenis pertanyaan berubah secara acak.' },
];

const quizCounts: QuizCount[] = [10, 20, 30, 'all'];

function QuizView({ items, onRate }: { items: HiraganaWithProgress[]; onRate: (id: string, rating: 0 | 1 | 2 | 3) => Promise<unknown> }) {
  ...
  if (mode === 'matching') {
    ...
    return <div className="quiz-shell">
      ...
      <div className="quiz-question-card quiz-matching-card">
        <p>Cocokkan Kana dengan romaji.</p>
        ...
        <small className="matching-hint">Pilih Kana di kiri, lalu pilih romaji pasangannya di kanan.</small>
...
function quizKindLabel(kind: QuizAttempt['kind']) {
  if (kind === 'kana-romaji') return 'Kana → Romaji';
  if (kind === 'romaji-kana') return 'Romaji → Kana';
...
function QuizPrompt({ question }: { question: QuizQuestion }) {
  ...
  if (question.kind === 'audio') {
    return <>
      <p>Dengarkan bunyi, lalu pilih Hiragana yang benar.</p>
...
function buildMatchingRounds(deck: HiraganaWithProgress[]): MatchingRound[] {
  if (!deck.length) return [];
  ...
  for (let roundIndex = 0; roundIndex < roundCount; roundIndex += 1) {
    ...
    const roundItems: HiraganaWithProgress[] = [];
...
function buildReadingOptions(correct: HiraganaWithProgress, pool: HiraganaWithProgress[]) {
...
function buildKanaOptions(correct: HiraganaWithProgress, pool: HiraganaWithProgress[]) {
...
function EmptyKana({ text = 'Belum ada data Hiragana pada kelompok ini.' }: { text?: string }) {

-- KOJAC LMS v2 - Milestone 2A hotfix
-- Memperbaiki ambiguity kolom item_id di RPC record_hiragana_review.
-- Jalankan SETELAH 002_hiragana_learning.sql.
...
begin
  ...
  if not found then
    ...
    v_new_interval := case p_rating
      when 0 then 0
      when 1 then 1
      when 2 then 1
      else 4
    end;
    v_new_mastery := case p_rating
      when 0 then 0
      when 1 then 5
      when 2 then 12
      else 18
